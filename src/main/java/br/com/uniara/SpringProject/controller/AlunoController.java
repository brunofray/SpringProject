...
public class AlunoController {
	...
	@PatchMapping("/")
	public ResponseEntity<String> alterar(@RequestBody AlunoEntity aluno){
	...
	public ResponseEntity<Map<String, String>> exception(Exception e){
		...
		return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(mapException);
	}
	@ExceptionHandler
	public ResponseEntity<Map<String, String>> exceptionRuntime(RuntimeException e){
		Map<String, String> mapException = new HashMap<>();
		mapException.put("message", e.getMessage());
		return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(mapException);
	}
}
